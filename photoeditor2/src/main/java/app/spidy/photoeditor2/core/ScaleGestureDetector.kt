...
import app.spidy.photoeditor2.core.ScaleGestureDetector.OnScaleGestureListener
...
class ScaleGestureDetector(private val mListener: OnScaleGestureListener) {
    ...
    open class SimpleOnScaleGestureListener : OnScaleGestureListener {
        ...
        override fun onScaleEnd(view: View?, detector: ScaleGestureDetector) { // Intentionally empty
        }
    }
    ...
    fun onTouchEvent(view: View, event: MotionEvent): Boolean {
        ...
        if (mInvalidGesture) {
            ...
            when (action) {
                ...
                MotionEvent.ACTION_POINTER_DOWN -> {
                    ...
                    if (index0 < 0 || index0 == index1) { // Probably someone sending us a broken event stream.
        ...
        } else { // Transform gesture in progress - attempt to handle it
            when (action) {
                MotionEvent.ACTION_POINTER_DOWN -> {
                    ...
                    mActive0MostRecent = false
                    var index0 = event.findPointerIndex(mActiveId0)
                    if (index0 < 0 || mActiveId0 == mActiveId1) { // Probably someone sending us a broken event stream.
                        index0 = findNewActiveIndex(event, mActiveId1, -1)
                        mActiveId0 = event.getPointerId(index0)
                    }
                    setContext(view, event)
